...
class Root(BoxLayout):
    ...
    def _worker(self):
        ...
        subprocess.run([os.path.join('assets','yt-dlp_linux_aarch64'),
                        "-f",fmt,"-o","tmp.%(ext)s",url])
        infile = next(f for f in os.listdir('.') if f.startswith("tmp."))
        outfile = out

        self.status = "Trimming..."
        ffmpeg_path = os.path.join('assets','ffmpeg')
        if outfile.endswith(".m4a"):
            subprocess.run([ffmpeg_path,"-y","-ss",start,"-to",end,"-i",infile,
